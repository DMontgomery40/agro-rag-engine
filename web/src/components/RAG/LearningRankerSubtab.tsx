...
export function LearningRankerSubtab() {
  ...
  useEffect(() => {
    if (!loading) {
      ...
      setTrainMaxLength(get('RERANKER_TRAIN_MAX_LENGTH', 512));
      setTrainLearningRate(get('RERANKER_TRAIN_LR', 0.00002));
      setWarmupRatio(get('RERANKER_WARMUP_RATIO', 0.1));
      setTripletsMinCount(get('TRIPLETS_MIN_COUNT', 100));
    }
  }, [loading, get]);
  ...
  const handleRerankerEnabledChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    set('AGRO_RERANKER_ENABLED', value);
  };
  ...
  const handleMineModeChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const value = e.target.value;
    setMineMode(value);
    set('AGRO_RERANKER_MINE_MODE', value);
  };
  ...
  const handleTrainEpochsBlur = () => {
    set('RERANKER_TRAIN_EPOCHS', trainEpochs);
  };
  ...
  const handleTrainBatchSizeBlur = () => {
    set('RERANKER_TRAIN_BATCH', trainBatchSize);
  };
  ...
  const handleTrainMaxLengthBlur = () => {
    set('RERANKER_TRAIN_MAX_LENGTH', trainMaxLength);
  };
  ...
  const handleTrainLearningRateBlur = () => {
    set('RERANKER_TRAIN_LR', trainLearningRate);
  };
  ...
  const handleWarmupRatioBlur = () => {
    set('RERANKER_WARMUP_RATIO', warmupRatio);
  };
        ...
        <div className="input-row">
          <div className="input-group">
            ...
            <input
              ...
              value={get('AGRO_RERANKER_ALPHA', 0.7)}
              min="0.0"
              max="1.0"
              step="0.05"
              onChange={(e) => set('AGRO_RERANKER_ALPHA', parseFloat(e.target.value))}
            />
          </div>
          <div className="input-group">
            <label>
          ...
          <div className="input-group">
            ...
            <input
              ...
              value={get('AGRO_RERANKER_BATCH', 16)}
              min="1"
              max="64"
              step="4"
              onChange={(e) => set('AGRO_RERANKER_BATCH', parseInt(e.target.value, 10))}
            />
          </div>
        </div>
