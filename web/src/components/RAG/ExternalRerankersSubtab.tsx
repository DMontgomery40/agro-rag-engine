...
export function ExternalRerankersSubtab() {
  ...
  const handleAddRerankModel = async () => {
    ...
    const modelInput = window.prompt('Rerank model ID to add to models.json:', cloudModel || '');
    ...
    <>
      ...
      <div className="settings-section" style={{ borderLeft: '3px solid var(--accent)', padding: '24px' }}>
        ...
        <p style={{ fontSize: '13px', color: 'var(--fg-muted)', marginBottom: '20px', lineHeight: '1.6' }}>
          ...
          an alternative local model, or an external API service. Cloud providers and models are sourced live from <code style={{ background: 'var(--card-bg)', padding: '2px 6px', borderRadius: '3px' }}>models.json</code> via <code style={{ background: 'var(--card-bg)', padding: '2px 6px', borderRadius: '3px' }}>/api/models</code>—no hardcoded lists.
        </p>
          ...
          <div className="input-group">
            ...
            >
              {providerOptions.length === 0 ? (
                <option value="">Add a rerank provider in models.json</option>
              ) : (
          ...
          <div className="input-group">
            ...
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
              ...
              <button
                ...
                data-tooltip="Add this provider/model to models.json and save it"
                style={{
                  padding: '8px 12px',
                  borderRadius: '6px',
        ...
        <div style={{ padding: '16px', background: 'rgba(64, 160, 255, 0.05)', border: '1px solid rgba(64, 160, 255, 0.2)', borderRadius: '8px' }}>
          ...
          <p style={{ fontSize: '12px', color: 'var(--fg-muted)', marginBottom: '16px', lineHeight: '1.5' }}>
            Use cloud-based reranking APIs instead of local models. Provider/model options come from <code style={{ background: 'var(--card-bg)', padding: '2px 6px', borderRadius: '3px' }}>models.json</code>; add entries above to extend the catalog without code changes.
          </p>

          <div className="input-row">
          <div className="input-group">
            <label>
              Cloud Model (models.json, provider-scoped)
              <span className="help-icon" data-tooltip="RERANKER_CLOUD_MODEL">?</span>
            </label>
            <select
              name="RERANKER_CLOUD_MODEL"
