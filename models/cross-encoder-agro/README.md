...
pairs = [
    ...
    ['Where is the Stemmer imported?', 'import os\nfrom fastapi import APIRouter\nfrom server.services.config_registry import get_config_registry\nfrom .learning_reranker import get_reranker, get_reranker_info\nfrom retrieval.rerank import get_rerank_config_info\n\nrouter = APIRouter()\n_config_registry = get_config_registry()\n\n@router.get("/api/reranker/info")'],
...
ranks = model.rank(
    'Where is config_registry defined?',
    [
        'import os\nfrom fastapi import APIRouter\nfrom server.services.config_registry import get_config_registry\nfrom .learning_reranker import get_reranker, get_reranker_info\nfrom retrieval.rerank import get_rerank_config_info\n\nrouter = APIRouter()\n_config_registry = get_config_registry()\n\n@router.get("/api/reranker/info")',
...
* Approximate statistics based on the first 100 samples:
  ...
  | details | <ul><li>min: 27 characters</li><li>mean: 38.9 characters</li><li>max: 59 characters</li></ul> | <ul><li>min: 180 characters</li><li>mean: 479.25 characters</li><li>max: 500 characters</li></ul> | <ul><li>min: 0.0</li><li>mean: 0.2</li><li>max: 1.0</li></ul> |
* Samples:
  | sentence_0                                                              | sentence_1                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | label            |
  |:------------------------------------------------------------------------|:----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|:-----------------|
  | <code>Where is config_registry defined?</code>                          | <code>import os<br>from fastapi import APIRouter<br>from server.services.config_registry import get_config_registry<br>from .learning_reranker import get_reranker, get_reranker_info<br>from retrieval.rerank import get_rerank_config_info<br><br>router = APIRouter()<br>_config_registry = get_config_registry()<br><br>@router.get("/api/reranker/info")</code>                                                                                                                                                                                                                                            | <code>0.0</code> |
