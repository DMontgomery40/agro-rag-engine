...
1. ✅ Direct Python invocation: WORKS
...
1. **Broken pipe**: "Do you want me to keep debugging (6+ hrs, uncertain) or work around it (2 hrs, guaranteed)?"
2. **Chat persistence**: "Should chat settings be per-browser (localStorage) or shared across users (.env)?"
3. **Model picker scope**: "Should I do all model fields at once or incrementally (chat first, then RAG, then admin)?"
4. **Testing priority**: "Do you want Playwright tests immediately or after features are stable?"
